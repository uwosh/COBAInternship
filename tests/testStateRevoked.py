...
class TestStateRevoked(COBAInternshipTestCase):
    ...
    def test_other_should_not_be_able_to_revoke(self):
        self.login(self._default_user)
        self.portal.invokeFactory(type_name="COBAInternship", id="testapplication")
        ...
        for user in self._all_users:
            if user != 'InternshipDirector' and user != 'Manager':
                self.login(user)
                self.assertRaises(WorkflowException, self.portal_workflow.doActionFor, app, 'revokeInternship')
                self.logout()               
...
def test_suite():
    ...
    suite = TestSuite()
    suite.addTest(makeSuite(TestStateRevoked))
